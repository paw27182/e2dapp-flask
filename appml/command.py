...
import appml.algo.cls_compare_algorithms as cca
import appml.algo.cls_find_optimal_model as cfom
import appml.algo.reg_compare_algorithms as rca
import appml.algo.reg_find_optimal_model as rfom
import appml.algo.statistics_analysis as sa
from appmain import command as appmain_cmd

mid = Path(__file__).name


def get_a_list_of_forms_for_machine_learning():
    print(f"[{mid}] get_a_list_of_forms_for_machine_learning")

    items = appmain_cmd.get_a_list_of_forms()
...
def prepare_working_dir(user):
    working_dir = os.path.join(current_app.config["BASE_DIR"], "appml/static/wdir", user)
    ...
    if rename_option:
        ...
        for p in Path(output_dir).resolve().iterdir():
            if p.suffix in [".png", ".html"]:
                ...
                x = str(p.parent) + "/" + str(mtime) + "_p_" + p.name  # _prefix_
                os.rename(p, x)
...
def machine_learning_dispatcher(command, selected_form, executeParameters):
    ...
    if "statistics_analysis" in command:
        ...
        user_log_file = Path(BASE_DIR, "appml/static/wdir", user, "user.log")
        ...
        # create command
        cmd_str = current_app.config["PYTHON_EXE_FILE"]
        BASE_DIR = current_app.config["BASE_DIR"]
